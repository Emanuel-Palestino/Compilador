    ...
    public static TablaLR construir(ColeccionCanonica coleccionCanonica, Gramatica gramatica) {
        ...
        for (int i = 0; i < coleccionCanonica.getConjuntosElementos().size(); i++) {
            ...
            for (Elemento recorreElemento : recorreConjunto.getElementos()) {
                ...
                if (gramatica.esTerminal(recorreElemento.getSimboloDespuesDePunto())) {
                    ConjuntoElementos aux = IrA.hacer(recorreConjunto, recorreElemento.getSimboloDespuesDePunto(),
                            gramatica);
                    acciones.get(i).replace(recorreElemento.getSimboloDespuesDePunto(),
                            "d" + coleccionCanonica.indiceDe(aux));
                    continue;
                ...
                if (recorreElemento.getIndexPunto() == (recorreElemento.getTamañoProduccion() - 1)) {
                    ...
                    if (!recorreElemento.getSimboloGramatical().equals(gramatica.getSimboloInicial() + "'")) {
                        for (String recorreSiguientes : resultadoSiguiente.get(0).getSimbolos()) {
                            if(gramatica.indiceReglaProduccion(temporalProduccion) == -1){
                                acciones.get(i).replace(recorreSiguientes, "r" + indiceEpsilon);
                            }else{
        ...
        for (int i = 0; i < coleccionCanonica.getConjuntosElementos().size(); i++) {
            for (int j = 0; j < gramatica.getNoTerminales().size(); j++) {
                ...
                temporal = IrA.hacer(coleccionCanonica.getConjuntosElementos().get(i),
                        gramatica.getNoTerminales().get(j), gramatica);
